...
Controller::Controller()
{
    ...
    desired_theta1 = -M_PI / 6;
    desired_theta2 = M_PI / 3;
    desired_omega1 = 0;
    desired_omega2 = 0;
    //////////////////////////////
...
Vector2d Controller::computedtorquemethod(double theta1_, double theta2_, double omega1_, double omega2_)
{
    ...
    Vector2d thetaVec(theta1, theta2);
    Vector2d desiredThetaVec(desired_theta1, desired_theta2);
    Vector2d omegaVec(omega1, omega2);
    Vector2d desiredOmegaVec(desired_omega1, desired_omega2);


    Vector2d tau = M * (KpMATRIX*(desiredThetaVec - thetaVec) + KdMATRIX * (desiredOmegaVec - omegaVec)) + G * 0.49;


    cout << thetaVec(0, 0) << "                " << thetaVec(1, 0) << endl;
    //cout << tau << ",      " << G * 0.47 << endl;
...
Vector2d Controller::leg2spring(double theta1_, double theta2_, double omega1_, double omega2_)
{
    ...
    // tau = J^T * F
    Vector2d tau = J.transpose() * F + G * 0.47;
